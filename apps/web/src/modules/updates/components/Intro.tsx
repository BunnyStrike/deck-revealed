    ...
    <>
      ...
      {title && (
        <h1 className='font-display mt-14 text-4xl/tight font-light text-white'>
          {title}{' '}
          {titleFocus && <span className='text-primary'>{titleFocus}</span>}
        </h1>
      )}
      {description && (
        <p className='mt-4 text-sm/6 text-gray-300'>{description}</p>
      )}
      <SignUpForm />
      <div className='mt-8 flex flex-wrap justify-center gap-x-1 gap-y-3 sm:gap-x-2 lg:justify-start'>
        ...
        {github && (
          <IconLink href={github} icon={GitHubIcon} className='flex-none'>
            GitHub
          </IconLink>
        )}
        {feed && (
