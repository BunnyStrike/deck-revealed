...
interface SyncIPCFunctions {
  ...
  showLogFileInFolder: (args: { appName: string; defaultLast?: boolean }) => void
...
interface AsyncIPCFunctions {
  ...
  refreshLibrary: (fullRefresh?: boolean, library?: Runner | 'all') => Promise<void>
  launch: (args: LaunchParams) => StatusPromise
  openDialog: (args: OpenDialogOptions) => Promise<string | false>
  install: (args: InstallParams) => Promise<{ status: 'error' | 'done' | 'abort' }>
  uninstall: (
    appName: string,
    runner: Runner,
  ...
  syncGOGSaves: (gogSaves: GOGCloudSavesLocation[], appname: string, arg: string) => Promise<string>
  ...
  removeApp: (args: { appName: string; shouldRemovePrefix: boolean }) => Promise<void>
  ...
  installWineVersion: (release: WineVersionInfo) => Promise<'error' | 'abort' | 'success'>
  ...
  addAppToSteam: (app: AppInfo, runner: Runner) => Promise<boolean>
  isAppAddedToSteam: (app: AppInfo, runner: Runner) => Promise<boolean>
  removeAppFromSteam: (app: AppInfo, runner: Runner) => Promise<void>
  removeFromSteam: (appName: string, runner: Runner) => Promise<void>
  isAddedToSteam: (appName: string, runner: Runner) => Promise<boolean>
  runScript: (appName: AppInfo) => Promise<boolean>
  runApp: (appName: AppInfo) => Promise<boolean>
  isAppInstalled: (appName: AppInfo) => Promise<boolean>
  uninstallApp: (appName: AppInfo) => Promise<boolean>
  isFlatpakInstalled: (source: string) => Promise<boolean>
  installBootVideo: (bootVideo: SteamOSBootVideo) => Promise<boolean>
  ...
  enableEosOverlay: (appName: string) => Promise<{ wasEnabled: boolean; installNow?: boolean }>
  ...
  getWikiGameInfo: (title: string, appName: string, runner: Runner) => Promise<WikiInfo | null>
  getDefaultSavePath: (
    appName: string,
    runner: Runner,
...
declare namespace Electron {
  class IpcMain extends EventEmitter {
    public on: <Name extends keyof SyncIPCFunctions, Definition extends SyncIPCFunctions[Name]>(
      name: Name,
    ...
    >(
      name: Name,
      callback: (e: IpcMainEvent, ...args: Parameters<Definition>) => ReturnType<Definition>
    ) => void
  }

  class IpcRenderer extends EventEmitter {
    public send: <Name extends keyof SyncIPCFunctions, Definition extends SyncIPCFunctions[Name]>(
      name: Name,
