...
import { gamepadAtom } from '../../states/gamepad'

export default function ControllerHints() {
  const [activeController] = useAtom(gamepadAtom)
  ...
  const setHintsFor = (target: HTMLElement) => {
    ...
    const list = target.closest('.listItem')
    ...
    if (card || list) {
      ...
      if (classes.contains('update-button')) {
        ...
        main = t('controller.hints.details', 'Details')
        alt2 = t('controller.hints.play', 'Play')
      } else {
        main = t('controller.hints.game_details', 'Details')
        alt2 = t('controller.hints.install', 'Install')
      }
  ...
  useEffect(() => {
    ...
    if (history?.state?.idx === 0) {
      ...
      setBackActionFallback(t('controller.hints.back', 'Back') ?? 'Back')
    }
  ...
  return (
    <div className={`controller-hints ${layout} bg-black`}>
      <div className='hint'>
