...
//       {
//         name: 'vkd3d',
//         url: 'https://api.github.com/repos/Revealed-Games-Launcher/vkd3d-proton/releases/latest',
...
//       const latestVersion = `${revealedToolsPath}/${tool.name}/${name}`
//       const pastVersionCheck = `${revealedToolsPath}/${tool.name}/latest_${tool.name}`
//       let pastVersion = ''
...
//       if (
//         pastVersion === pkg &&
//         existsSync(`${revealedToolsPath}/${tool.name}/${pkg}`)
//       ) {
//         return
//       }

//       const downloadCommand = `curl -L ${downloadUrl} -o '${latestVersion}' --create-dirs`
//       const extractCommand = `${tool.extractCommand} '${latestVersion}' -C '${revealedToolsPath}/${tool.name}'`
//       const echoCommand = `echo ${pkg} > '${revealedToolsPath}/${tool.name}/latest_${tool.name}'`
...
//     if (!existsSync(`${revealedToolsPath}/${tool}/latest_${tool}`)) {
//       logWarning('dxvk not found!', LogPrefix.DXVKInstaller)
//       await DXVK.getLatest()
//     }

//     const globalVersion = readFileSync(
//       `${revealedToolsPath}/${tool}/latest_${tool}`
//     )
//       .toString()
//       .split('\n')[0]

//     const dlls = readdirSync(
//       `${revealedToolsPath}/${tool}/${globalVersion}/x64`
//     )
//     const toolPathx32 = `${revealedToolsPath}/${tool}/${globalVersion}/${
//       tool === 'vkd3d' ? 'x86' : 'x32'
//     }`
//     const toolPathx64 = `${revealedToolsPath}/${tool}/${globalVersion}/x64`
...
//     const path = `${revealedToolsPath}/winetricks`
...
//       const file = res.data
//       writeFileSync(path, file, { mode: 0o755, flag: 'w' })
...
//       return
//     }
//     const winetricks = `${revealedToolsPath}/winetricks`
