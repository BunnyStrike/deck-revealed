...
import { parse as plistParse, type PlistObject } from 'plist'
...
abstract class GlobalConfig {
  ...
  public static get(): GlobalConfig {
    ...
    if (!existsSync(revealedConfigPath)) {
    ...
    // Config file exists, detect its version.
      ...
      try {
        version = JSON.parse(readFileSync(revealedConfigPath, 'utf-8'))[
          'version'
        ]
      } catch (error) {
        logError(
          `Config file is corrupted, please check ${revealedConfigPath}`,
          LogPrefix.Backend
        )
        version = 'v0'
      }
  ...
  public async getWineOnMac(): Promise<Set<WineInstallation>> {
    ...
    // search for wine installed on $HOME/Library/Application Support/revealed/tools/wine
    const wineToolsPath = `${revealedToolsPath}/wine/`
  ...
  public async getAlternativeWine(
    ...
    if (!existsSync(`${revealedToolsPath}/wine`)) {
      mkdirSync(`${revealedToolsPath}/wine`, { recursive: true })
    }

    if (!existsSync(`${revealedToolsPath}/proton`)) {
      mkdirSync(`${revealedToolsPath}/proton`, { recursive: true })
    }

    const altWine = new Set<WineInstallation>()

    readdirSync(`${revealedToolsPath}/wine/`).forEach((version) => {
      const wineBin = join(revealedToolsPath, 'wine', version, 'bin', 'wine')
      altWine.add({
        bin: wineBin,
        name: `Wine - ${version}`,
        type: 'wine',
    ...
    const protonPaths = [`${revealedToolsPath}/proton/`]
  ...
  protected writeToFile(config: Record<string, unknown>) {
    return writeFileSync(revealedConfigPath, JSON.stringify(config, null, 2))
  }
...
class GlobalConfigV0 extends GlobalConfig {
  ...
  public getSettings(): AppSettings {
    ...
    if (!existsSync(revealedGamesConfigPath)) {
      mkdirSync(revealedGamesConfigPath, { recursive: true })
    }
    ...
    let settings = JSON.parse(readFileSync(revealedConfigPath, 'utf-8'))
  ...
  public getFactoryDefaults(): AppSettings {
    const account_id = '' //TODO: add back in LegendaryUser.getUserInfo()?.account_id
    ...
    return {
      ...
      defaultInstallPath: revealedInstallPath,
      libraryTopSection: 'disabled',
      defaultSteamPath: getSteamCompatFolder(),
      defaultWinePrefix: revealedDefaultWinePrefix,
  ...
  public setSetting(key: string, value: unknown) {
    const config = this.getSettings() as any
    ...
    config[key] = value
    this.config = config
    logInfo(`Revealed: Setting ${key} to ${JSON.stringify(value)}`)
    return this.flush()
  }
