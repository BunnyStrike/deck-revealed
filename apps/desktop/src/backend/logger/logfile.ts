import { existsSync, openSync, readdirSync, unlinkSync, appendFileSync } from 'graceful-fs'

import { configStore, currentLogFile, revealedGamesConfigPath, lastLogFile } from '../constants'
...
/**
 * Creates a new log file in revealed config path under folder Logs.
...
export function createNewLogFileAndClearOldOnes(): createLogFileReturn {
  ...
  const newLogFile = join(logDir, `revealed-${fmtDate}.log`)
  ...
  if (existsSync(logDir)) {
    try {
      ...
      logs.forEach((log) => {
        if (log.match(/(revealed|legendary|gogdl)-/)) {
          const dateString = log
            .replace(/(revealed|legendary|gogdl)-/, '')
            .replace('.log', '')
            .replaceAll('_', ':')
 ...
 * @param defaultLast if set getting revealed default last log
 * @returns path to log file
 */
export function getLogFile(props: { appName?: string; defaultLast?: boolean }): string {
  return props.appName
    ? join(revealedGamesConfigPath, props.appName + '-lastPlay.log')
    : props.defaultLast
    ? lastLogFile
    : currentLogFile
}
