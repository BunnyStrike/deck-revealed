...
describeSkipOnWindows('logger/logfile.ts', () => {
  ...
  test('createNewLogFileAndClearOldOnes fails because logDir does not exist', () => {
    ...
    expect(spyOpenSync).toBeCalledWith(expect.stringContaining('invalid/revealed-'), 'w')
    expect(spyAppGetPath).toBeCalledWith('logs')
    expect(logError).toBeCalledWith(
      [
        expect.stringContaining(`Open invalid/revealed-`),
        expect.objectContaining(
          Error("ENOENT: no such file or directory, open 'invalid/revealed-")
        ),
      ],
  ...
  test('createNewLogFileAndClearOldOnes removing old logs fails', () => {
    ...
    const monthOutdatedLogFile = join(
      tmpDir.name,
      `revealed-${date.toISOString().replaceAll(':', '_')}.log`
    )
    ...
    expect(logError).toBeCalledWith(
      [expect.stringContaining('Removing old logs in /tmp/'), Error('unlink failed')],
      { prefix: 'Backend', skipLogToFile: true }
    )
    expect(graceful_fs.existsSync(monthOutdatedLogFile)).toBeTruthy()
  })
  ...
  test('getLogFile all possible values', () => {
    ...
    expect(logfile.getLogFile({ appName: 'MyApp' })).toBe(
      '/tmp/appData/revealed/GamesConfig/MyApp-lastPlay.log'
    )
  ...
  test('appendMessageToLogFile success', () => {
    const appendFileSyncSpy = jest.spyOn(graceful_fs, 'appendFileSync').mockReturnValue()
  ...
  test('appendMessageToLogFile fails', () => {
    ...
    logfile.appendMessageToLogFile('Hello World')
    expect(logError).toBeCalledWith(['Writing log file failed with', Error('append failed')], {
      prefix: 'Backend',
      skipLogToFile: true,
    })
  })
