...
async function prepareImagesForSteam(props: {
  ...
  if (errors.length > 0) {
    logError(
      [
        `Preparing Steam images for ${title || name} failed with:\n`,
        errors.join('\n'),
      ],
      LogPrefix.Shortcuts
    )
...
function removeImagesFromSteam(props: {
  ...
  const { title, name } = props.gameInfo as any

  if (!existsSync(gridFolder)) {
    return
  }

  logInfo(`Remove Steam images for ${title ?? name}`, LogPrefix.Shortcuts)
  ...
  if (errors.length > 0) {
    logError(
      [
        `Removing Steam images for ${title ?? name} failed with:\n`,
        errors.join('\n'),
      ],
      LogPrefix.Shortcuts
    )
